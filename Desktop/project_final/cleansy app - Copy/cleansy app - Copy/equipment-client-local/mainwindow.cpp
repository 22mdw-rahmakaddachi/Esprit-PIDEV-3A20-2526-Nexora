    ...
    , currentImageIndex(0)  // Initialize currentImageIndex correctly here
{
    ...
    // Connect the clicked signal of the client_tableView to the slot
        ...
        ///arduino
            ...
            /////////////arduino1
            ///////////arduino1
            // Créer une instance de QSerialPort
            serialPort = new QSerialPort(this);
            flameDetected = false;

            // Connexion du bouton pour vérifier la connexion à l'Arduino secondaire
            connect(ui->checkButton_2, &QPushButton::clicked, this, &MainWindow::on_checkButton_2_clicked);

            // Configuration d'un timer pour lire les données de l'Arduino secondaire périodiquement
            QTimer *timer = new QTimer(this);
            connect(timer, &QTimer::timeout, this, &MainWindow::readFromArduino1);
            timer->start(100); // Lire toutes les 100 ms

            // Connecter d'autres boutons si nécessaire
...
{
    int ID = ui->idLineEdit_2->text().toInt();
    QString ADRESSE = ui->adresseLineEdit_2->text().trimmed();
    QString TYPE = ui->typeLineEdit_2->text().trimmed();
    ...
    if (TYPE.isEmpty()) {
        QMessageBox::warning(this, "Erreur", "Le type ne peut pas être vide.");
        return;
    }

    Local local(ID, ADRESSE, TYPE);

    if (local.ajouterLocal()) {
        QMessageBox::information(this, "Succès", "Le local a été ajouté avec succès !");
        ui->idLineEdit_2->clear();
        ui->adresseLineEdit_2->clear();
        ui->typeLineEdit_2->clear();
    } else {
        QMessageBox::critical(this, "Erreur", "Échec de l'ajout du local.");
    }
}
...
{
    // Récupération des valeurs saisies pour la modification
    int ID = ui->idLineEdit_2->text().toInt();
    QString ADRESSE = ui->adresseLineEdit_2->text().trimmed();
    QString TYPE = ui->typeLineEdit_2->text().trimmed();
    ...
    if (local.modifierLocal()) {
        QMessageBox::information(this, "Succès", "Le local a été modifié avec succès !");

        // Effacer les champs après une modification réussie
        ui->idLineEdit_2->clear();
        ui->adresseLineEdit_2->clear();
        ui->typeLineEdit_2->clear();

        // Rafraîchir les données dans la table
        QSqlQueryModel *model = local.afficherLocal();
        ui->tableView_2->setModel(model); // Mettre à jour le QTableView avec les nouvelles données
    } else {
        QMessageBox::critical(this, "Erreur", "Échec de la modification du local.");
    }
}
...
{
    QSqlQuery query;
    query.prepare("SELECT TYPE, COUNT(*) FROM local GROUP BY TYPE");
    ...
    QChart *chart = new QChart();
    chart->addSeries(series);
    chart->setTitle("Statistiques des Type par TYPE");
...
void MainWindow::on_pushButton_11_clicked()
{
    ...
    // Create a PDF file (regler le fichier pdf plus jolie et organise)
    ...
    QFont font;
    font.setPointSize(14); // Adjust font size for the title
    painter.setFont(font);

    // Draw the title "Fichier des Locaux" at the top of the page
    int titleX = 100; // X position for the title
    int titleY = 50;  // Y position for the title (distance from the top of the page)
    painter.drawText(titleX, titleY, "Fichier des Locaux");

    // Increase Y offset after drawing the title
    int yOffset = 100; // Adjust this based on the title height

    // Set the font for the table headers and data
    font.setPointSize(10); // Adjust font size for the table
    painter.setFont(font);

    // Fetch the data from your database (replace this with your actual query)
    QSqlQueryModel model;
    model.setQuery("SELECT * FROM local"); // Modify the SQL query accordingly

    // Define column widths and titles
    QVector<int> columnWidths = {400, 485, 410}; // Adjust widths as needed
    QStringList columnTitles = {"ADRESSE", "TYPE", "ID"};
    ...
    yOffset += 30; // Add space below the header row
...
{
    Local l;
    int ID = ui->lineEdit_3->text().toInt(); // Check if lineEdit has a valid integer
    if (l.supprimerLocal(ID)) {
        ui->tableView_2->setModel(l.afficherLocal());
        QMessageBox::information(this, "Succès", "Adresse supprimée avec succès.");
    } else {
        QMessageBox::critical(this, "Erreur", "Échec de la suppression de l'adresse.");
    }
}
...
void MainWindow::on_pushButton_10_clicked()
{
    ...
    if (ID == 0)
    {
        searchResultsModel = l.afficherLocal();
    }
    else
    {
        QSqlQuery query;
        ...
        {
            QMessageBox::critical(this, "Erreur", "Echec de l'exécution de la requête.");
        }
    }

    ui->tableView_2->setModel(searchResultsModel);
}
...
void MainWindow::on_pushButton_13_clicked()
{
    ...
    if (ui->radioButton_7->isChecked()) {
        model = l.tri_par_adresse_croissant();
    } else if (ui->radioButton_8->isChecked()) {
        model = l.tri_par_adresse_decroissant();
    } else if (ui->radioButton_5->isChecked()) {
        model = l.tri_par_id_croissant();
    } else if (ui->radioButton_6->isChecked()) {
        model = l.tri_par_id_decroissant();
    }
...
{
    // Récupérer l'ID saisi par l'utilisateur (s'il existe)
    int ID = ui->lineEdit_4->text().toInt(); // Assurez-vous que le champ lineEdit_4 contient un ID valide

    if (ID == 0) {
        // Si l'ID est égal à 0, il y a une erreur dans l'entrée
        QMessageBox::warning(this, "Erreur", "Veuillez entrer un ID valide.");
        return;
    }

    // Créer une réponse vide par défaut
    QString response;

    // Vérifier si la demande concerne l'adresse
    if (ui->lineEdit_4->text().contains("adresse", Qt::CaseInsensitive)) {
        // Créer une requête SQL pour récupérer l'adresse associée à l'ID donné
        QSqlQuery query;
        query.prepare("SELECT ADRESSE FROM local WHERE ID = :id");
        query.bindValue(":id", ID);

        // Exécuter la requête
        if (query.exec() && query.next()) {
            // Si la requête a réussi et un résultat a été trouvé, récupérer l'adresse
            QString adresse = query.value("ADRESSE").toString();
            response = QString("L'adresse du local avec ID %1 est : %2").arg(ID).arg(adresse);
        } else {
            // Si l'ID n'existe pas ou la requête échoue, afficher un message d'erreur
            response = QString("Désolé, aucun local trouvé avec l'ID %1.").arg(ID);
        }
    }
    // Vérifier si la demande concerne le type
    else if (ui->lineEdit_4->text().contains("type", Qt::CaseInsensitive)) {
        // Créer une requête SQL pour récupérer le type associé à l'ID donné
        QSqlQuery query;
        query.prepare("SELECT TYPE FROM local WHERE ID = :id");
        query.bindValue(":id", ID);

        // Exécuter la requête
        if (query.exec() && query.next()) {
            // Si la requête a réussi et un résultat a été trouvé, récupérer le type
            QString type = query.value("TYPE").toString();
            response = QString("Le type du local avec ID %1 est : %2").arg(ID).arg(type);
        } else {
            // Si l'ID n'existe pas ou la requête échoue, afficher un message d'erreur
            response = QString("Désolé, aucun local trouvé avec l'ID %1.").arg(ID);
        }
        ...
        // Si la demande n'est pas liée à l'adresse ou au type, afficher un message générique
        response = "Je suis désolé, je n'ai pas compris votre demande. Veuillez demander l'adresse ou le type d'un local.";
    }

    // Afficher la réponse du chatbot dans la zone de texte (textEdit)
    ui->textEdit_4->append("Chatbot: " + response);

    // Optionnellement, effacer la zone de texte de l'entrée utilisateur
    ui->textEdit_5->clear();
}
/////arduinooooooooooo11111111


void MainWindow::readFromArduino1()
{
    // Lire les données envoyées par l'Arduino
    if (serialPort->canReadLine()) {
        QByteArray data = serialPort->readLine();
        bool ok;
        int flameValue = data.trimmed().toInt(&ok);

        if (ok) {
            flameDetected = (flameValue < 500);  // Comparer avec le seuil
            updateFlameStatus();
        } else {
            qDebug() << "Erreur de conversion des données: " << data;
        }
    }
}

void MainWindow::updateFlameStatus()
{
    // Mettre à jour l'affichage du QLabel selon l'état de la détection
    QSqlQuery query;
    QString a;
    if (flameDetected) {
        ui->flameStatusLabel_2->setText("DANGER: Flame detected!");
        ui->flameStatusLabel_2->setStyleSheet("background-color: red; color: white;");
        a = "1";
    } else {
        ui->flameStatusLabel_2->clear();
        ui->flameStatusLabel_2->setStyleSheet("background-color: none;");
        a ="0";
    }
    query.prepare("UPDATE LOCAL SET FLAM = :code");
    query.bindValue(":code", a);
...
{
    // Si le port série est ouvert, on le ferme
    if (serialPort->isOpen()) {
        serialPort->close();  // Ferme la connexion série
    }

    // Appel à la méthode de base pour fermer la fenêtre proprement
    QMainWindow::closeEvent(event);  // Cette ligne ferme effectivement la fenêtre
}
...
void MainWindow::on_checkButton_2_clicked()
{
    ...
    QString portName = "COM5";  // Remplacez cela par le port réel de votre Arduino
    bool portFound = false;

    for (const QSerialPortInfo &port : availablePorts) {
        qDebug() << "Port disponible:" << port.portName();
    ...
    if (!portFound) {
        QMessageBox::warning(this, "Arduino Connection", "Le port " + portName + " n'est pas disponible.");
        return;
    }
    ...
    if (serialPort->open(QIODevice::ReadWrite)) {
        QMessageBox::information(this, "Arduino Connection", "Successfully connected to port: " + portName);
    } else {
