...
final class PublicationController extends AbstractController
{
    ...
    {
        // Si connecté, on force le nom de l'utilisateur
        $user = $this->getUser();
    ...
    public function commenter(int $id, Request $request, Connection $conn): JsonResponse
    {
        ...
        // Récupérer l'auteur de la publication pour la notifier
        $pub = $conn->fetchAssociative('SELECT auteur FROM publication WHERE id = ?', [$id]);
        $this->notif->push(
            type:       'commentaire',
            message:    "{$auteur} a commenté votre publication",
            actor:      $auteur,
            targetUser: $pub['auteur'] ?? null,
            refId:      $id,
            refType:    'publication'
        );
    ...
    {
        $user   = $this->getUser();
        $auteur = $user ? $user->getFullName() : trim($request->request->get('auteur', 'Anonyme'));
        ...
        // Notifier l'auteur de la publication (seulement si nouvelle réaction)
        if (!$existing) {
            $pub = $conn->fetchAssociative('SELECT auteur FROM publication WHERE id = ?', [$id]);
            $emojis = ['jaime'=>'👍','jadore'=>'❤️','haha'=>'😂','wow'=>'😮','triste'=>'😢','grrr'=>'😡'];
            $this->notif->push(
                type:       'reaction',
                message:    "{$auteur} a réagi {$emojis[$type]} à votre publication",
                actor:      $auteur,
                targetUser: $pub['auteur'] ?? null,
                refId:      $id,
                refType:    'publication'
